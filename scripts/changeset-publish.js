...
async function uploadAssetStream({ owner, repo, release_id, filepath }) {
  ...
  for (const release of releases) {
    ...
    const checksums = fs
      ...
      .map((artifactName) => {
        ...
        return { artifactName, resolvedArtifactPath, checksum512, digest256 };
      });
    ...
    const options = {
      ...
      name: releaseName,
      body: `*checksums*\n\n${bodyText}`,
      assets: checksums.map((artifact) => artifact.resolvedArtifactPath),
    };
    ...
    const subjects = checksums.map(({ artifactName, digest256 }) => ({ name: artifactName, digest: { sha256: digest256 } }));
